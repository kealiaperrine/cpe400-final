...
def fail_nodes(graph, fail_probs, num_nodes):
    ...
    failed = []
    total_fail = random.randint(0, num_nodes-2)
...
def update_weights(graph, fail_probs):
    ...
    print(avg)
    for (x, (u, v)) in zip(fail_probs, graph.edges()):
        print ("okie")

    

# plots and prints the generate graph, use python libraries networkx and matplotlib
# TO-DO: not have to close graph to continue program?5
...
def main():
    ...
    print("This is your randomly created graph, before router failures:")
    show_graph(graph)
